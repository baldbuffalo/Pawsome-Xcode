...
struct ScanView: View {
    ...
    private func pickFile() {
        ...
        if panel.runModal() == .OK, let url = panel.urls.first, let img = PlatformImage(contentsOf: url) {
            selectedImage = img
            showForm = true
        }
        #elseif os(iOS)
        // On iOS we would use UIImagePickerController
        // Here we just simulate file pick
        showForm = true
        #endif
    }
}
